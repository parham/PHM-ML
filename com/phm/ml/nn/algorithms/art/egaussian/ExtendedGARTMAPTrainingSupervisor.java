 ...
 * @author phm
 */
public class ExtendedGARTMAPTrainingSupervisor extends GaussianARTMAPTrainingSupervisor {
    
    protected EGARTLearningRateCalculator laPolicy;
    
    public ExtendedGARTMAPTrainingSupervisor () {
       this (new DefaultLRC ());
    }
    public ExtendedGARTMAPTrainingSupervisor (EGARTLearningRateCalculator lp) {
        laPolicy = Objects.requireNonNull(lp);
    }
    
    @Override
    public String getName() {
        return "egartmap.train";
    }
    @Override
    protected void update(NeuronGroup ngroup, List<Neuron> neuron, Instance signal) {
        ...
        for (int dim = 0; dim < stddim.length; dim++) {
            ...
            stddim [dim] = (float) Math.sqrt (((1 - glearn) * (stddim[dim] * stddim[dim])) + 
                                     (glearn * (signal.value(dim) - win.value(dim))
