 ...
 * @author phm
 */
public class Artificial2DPGARTMAPMain {

    public static String getClassWithHighestProbability (HashMap<String, Float> clss) {
        if (clss != null) {
            LinkedList<String> keys = new LinkedList<>(clss.keySet());
            String maxl = keys.get(0);
            float maxv = clss.getOrDefault(maxl, 0.0f);
            for (String k : keys) {
                float tmp = clss.getOrDefault(k, 0.0f);
                if (tmp > maxv) {
                    maxl = k;
                    maxv = tmp;
                }
    ...
    public static void main(String[] args) {
        ...
        ProbabilisticGARTMAPTrainingSupervisor gart = new ProbabilisticGARTMAPTrainingSupervisor ();
...
//            Neuron tmp = new Neuron(n);
//            String lbl = (String) n.classValue();
//            tmp.setClassValue(lbl);
//            resds.add(tmp);
//            
//        }
