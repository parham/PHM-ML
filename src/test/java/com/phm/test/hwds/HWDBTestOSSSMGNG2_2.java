 ...
 * @author phm
 */
public class HWDBTestOSSSMGNG2_2 {
...
//            int id = rand.nextInt(db.size()-1);
//            DataField<float []> df = db.remove (id);
//            dbtest.add(df);
//            //db.get(id).label = OSSSMGNGTrainSupervisor2.OSSSMGNG_DEFAULT_UNLABEL;
//        }
...
//            int rin = 0;
//            if (dbtest.size() > 1) {
//                rin = r.nextInt(dbtest.size()-1);
//            }
//            DataField<float []> tmp = dbtest.remove(rin);
...
//        runtime -= System.currentTimeMillis();
//        System.out.println ("Runtime --> " + Math.abs (runtime));
//        System.out.println ("RATE: " + (float) numtrue / dbsize);
//        
//        try {
