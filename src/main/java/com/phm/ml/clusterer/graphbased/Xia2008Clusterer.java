 ...
 * </b> <b>Authors:</b> Xia, Y., & Peng, X. <br>
 * <b>Year:</b> 2008 <br>
 * <b>Title:</b> A clustering algorithm based on Delaunay Triangulation <br>
 * <b>Published In:</b> 7th World Congress on Intelligent Control and Automation
 * <br>
 * <b>Page:</b> 4517 – 4521 <br>
 * <b>DOI:</b> 10.1109/WCICA.2008.4593651 <br>
 * <b>Abstract :</b> Most clustering methods require user-specified parameters
 * or prior knowledge to produce their best results, this demands pre-processing
 * or several trials. Both are extremely expensive and inefficient, because the
 * best-fit parameters are not easy to get. This paper presents a new approach
 * (CBDTM) which is on the basis of Delaunay Triangulation. This approach
 * introduces the median length of k-nearest edges as measure to divide edges
 * for each point. The parameters of CBDTM are not specified by users, and the
 * experiment shows to us that it can find different shape clusters not only in
 * different density data sets, but also in data sets with noise. All operations
 * complete within expected time O(nlogn) , where n is the number of the data
 * sets. The performance comparison experiments show to us, CBDTM more efficient
 * and it has better quality than AUTOCLUST. <br>
 * </p>
 * 
...
public class Xia2008Clusterer<V extends Instance, E extends Edge>
	...
	protected HashMap<V, Double> localStdForAllVerteces(Graph<V, E> g, HashMap<E, Double> dis,
	...
	protected double localMeanEdges(Graph<V, E> g, V n, HashMap<E, Double> dis) {
		LinkedList<E> es = new LinkedList<>(g.edgesOf(n));
		double res = 0;
		for (E e : es) {
			res += dis.get(e);
	...
	protected HashMap<V, Double> localMeanForAllVerteces(Graph<V, E> g, HashMap<E, Double> dis) {
	...
	protected HashMap<E, Boolean> determineEdgesType(Graph<V, E> g, V n, HashMap<E, Double> dis,
			HashMap<V, Double> meds, HashMap<V, Double> stds) {
	...
	protected HashMap<V, HashMap<E, Boolean>> determinesAllVerteces(Graph<V, E> g, HashMap<E, Double> dis,
			HashMap<V, Double> meds, HashMap<V, Double> stds) {
