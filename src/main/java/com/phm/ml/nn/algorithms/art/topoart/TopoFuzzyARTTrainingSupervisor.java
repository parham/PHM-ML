...
public class TopoFuzzyARTTrainingSupervisor extends FuzzyARTTrainingSupervisor {
	...
	public void update(NeuronGroup ngroup, List<Neuron> neuron, Instance signal) {
		...
		for (int index = 0; index < nAdims; index++) {
			nA.put(index, ((1 - learningRate) * nA.value(index))
					+ (learningRate * Math.min(nA.value(index), signal.value(index))));
		}
		...
		if (neuron.size() > 1) {
			...
			float learningRateB = (float) ngroup
					.getParameter(TopoFuzzyARTTrainingSupervisor.TOPOFUZZYART_LEARNING_RATE_B);
			for (int index = 0; index < nBdims; index++) {
				nB.put(index, ((1 - learningRateB) * nB.value(index))
						+ (learningRateB * Math.min(nB.value(index), signal.value(index))));
			}
		}
	}
	...
	protected boolean addInternalConnection(Neuron n1, Neuron n2) {
		if (!n1.connections.contains(n1, n2) && !n2.connections.contains(n2, n1)) {
	...
	protected boolean removeInternalConnection(Neuron n1, Neuron n2) {
		return n1.connections.remove(new Connection(n1, n2)) && n2.connections.remove(new Connection(n2, n1));
	}
	...
	public static class TopoARTbActivationPermission implements NeuronAnalyzer {
		...
		public void analysis(String state, Neuron n, ParametersContainer param, Instance current,
			...
			int numLearn = (int) result.getRecent().winners.get(0)
					.getParameter(TopoFuzzyARTTrainingSupervisor.NEURON_LEARNITEM_COUNTER);
