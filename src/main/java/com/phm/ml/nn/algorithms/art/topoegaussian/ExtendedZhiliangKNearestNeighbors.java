...
/**
 * Publication details: (Modified version by Parham Nooralishahi for Topological
 * GARTMAP) Authors: Zhiliang, L., Xiaomin, Z., Jianxiao, Z. & Hongbing, X.
 * Year: 2013 Title: A Semi-Supervised Approach Based on k-Nearest Neighbor.
 * Published In: Journal Of Software, VOL. 8, NO. 4 Page: 768 – 775
 * 
...
public class ExtendedZhiliangKNearestNeighbors extends WeightedKNearestNeighborsClassifier {
	...
	public synchronized Map<Object, Double> classDistribution(Instance df) {
		...
		for (KNNRecord tx : knearest) {
			...
			HashMap<Object, Double> cp = (HashMap<Object, Double>) n
					.getParameter(TopoEGARTTrainingSupervisor.NEURON_OCCURENCE_VECTOR);
	...
	protected boolean validate(LinkedList<KNNRecord> ds, HashMap<Object, Double> cd, Object cls) {
		double res = 0.0;
		double factor = 0.0;
		...
		for (KNNRecord x : ds) {
			Neuron n = (Neuron) (x.data);
			HashMap<Object, Double> ncd = (HashMap<Object, Double>) n
					.getParameter(TopoEGARTTrainingSupervisor.NEURON_OCCURENCE_VECTOR);
			factor += x.distance;
			double v = 0;
