...
//        }
	...
	protected List<Neuron> onFindHighestErrorNeurons(NeuronGroup ngroup, Instance s, List<Neuron> result) {
		...
		for (Neuron ns : neighbors) {
			...
			for (int index = 0; index < nst.noAttributes(); index++) {
				dist += ((nst.value(index) - q.value(index)) * (nst.value(index) - q.value(index)));
			}
	...
	protected void solveNewNeuronNeighbors(NeuronGroup ngroup, Neuron q, Neuron f, Neuron newn) {
		List<Neuron> neighbors = q.connections.neighbors();
	...
	protected NNResult prepareResult(Neuron neuron, Instance signal, List<Neuron> winners, NNResult resc) {
		...
		resc.setParameter(GCSTrainingSupervisor.GCS_EPSILON_B,
				neuron.getParameter(GCSTrainingSupervisor.GCS_EPSILON_B));
		resc.setParameter(GCSTrainingSupervisor.GCS_EPSILON_N,
				neuron.getParameter(GCSTrainingSupervisor.GCS_EPSILON_N));
		resc.setParameter(GCSTrainingSupervisor.GCS_LAMBA, neuron.getParameter(GCSTrainingSupervisor.GCS_LAMBA));
		resc.setParameter(GCSTrainingSupervisor.NEURON_DISTANCE,
				neuron.getParameter(GCSTrainingSupervisor.NEURON_DISTANCE));
		resc.setParameter(GCSTrainingSupervisor.NEURON_DIS_DIMENSION,
				neuron.getParameter(GCSTrainingSupervisor.NEURON_DIS_DIMENSION));
		resc.setParameter(GCSTrainingSupervisor.NEURON_LOCALERROR,
				neuron.getParameter(GCSTrainingSupervisor.NEURON_LOCALERROR));
		return resc;
	}
}
