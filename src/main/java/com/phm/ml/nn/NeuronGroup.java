...
public class NeuronGroup extends Neuron implements Graph<Neuron, Connection> {
	...
	public NeuronGroup(NeuronsContainer nc, ParametersContainer param, NeuronConnectionsContainer ncc, double[] dims) {
		super(param, ncc, dims);
		neurons = Objects.requireNonNull(nc);
		initialize();
	}

	public NeuronGroup(NeuronsContainer nc, ParametersContainer param, NeuronConnectionsContainer ncc, Instance dims) {
		super(param, ncc, dims);
		neurons = Objects.requireNonNull(nc);
		initialize();
	}

	public NeuronGroup(ParametersContainer param, NeuronConnectionsContainer cc, Instance chns) {
		this(new ListNeuronContainer(), param, cc, chns);
	}

	public NeuronGroup(ParametersContainer param, NeuronConnectionsContainer cc, double[] chns) {
		this(new ListNeuronContainer(), param, cc, chns);
	}
	...
	public boolean updateInternalConnection(Connection c) {
		if (c == null)
			return false;
		if (c.neuronOne.connections.add(c) && c.neuronTwo.connections.add(c)) {
	...
	public boolean removeInternalConnection(Connection c) {
		...
		if (c.neuronOne.connections.remove(c) && c.neuronTwo.connections.remove(c)) {
	...
	public boolean containsEdge(Connection c) {
		if (c.neuronOne.connections.contains(c) && c.neuronTwo.connections.contains(c)) {
			return true;
		}
		return false;
	}
