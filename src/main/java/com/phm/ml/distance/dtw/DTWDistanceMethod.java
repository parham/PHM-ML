 ...
 * </b> <b>Authors:</b> Kruskall, J. & M. Liberman <br>
 * <b>Year:</b> 1983 <br>
 * <b>Title:</b> The Symmetric Time Warping Problem: From Continuous to Discrete
 * <br>
 * <b>Published In:</b> Time Warps, String Edits and Macromolecules: The Theory
 * and Practice of Sequence Comparison <br>
 * <b>Page:</b> 125-161 <br>
 * </p>
 * 
...
public class DTWDistanceMethod extends DistanceMethod {
	...
	protected void afterCalculateCostMatrix(Instance sc1, Instance sc2, CostMatrix cm, ParametersContainer pc) {
		// Empty body
	}

	protected void beforeCalculateCostMatrix(Instance sc1, Instance sc2, ParametersContainer pc) {
		// Empty body
	}

	public CostMatrix onCalculateCostMatrix(Instance sc1, Instance sc2, ParametersContainer pc) {
		...
		for (int row = 1; row < maxRow; row++) {
			costMatrix.set(row, 0, costTemp[row][0] + costMatrix.get(row - 1, 0));
		}
		for (int col = 1; col < maxCol; col++) {
			costMatrix.set(0, col, costTemp[0][col] + costMatrix.get(0, col - 1));
		}
		for (int row = 1; row < maxRow; row++) {
			for (int col = 1; col < maxCol; col++) {
				costMatrix.set(row, col, costTemp[row][col] + Math.min(costMatrix.get(row - 1, col),
						Math.min(costMatrix.get(row - 1, col - 1), costMatrix.get(row, col - 1))));
			}
		}
		return costMatrix;
	}
	...
	protected List<DTWDistanceInfo.WarpingIndex> findNeighbors(SearchWindow win, int row, int col) {
		...
		if (doesMoveDirectToUp()) {
			...
			c = col;
			if (win.isValid(r, c))
				list.add(new DTWDistanceInfo.WarpingIndex(r, c, win.get(r, c)));
		}
	...
	public void beforeCalculateDTWDistance(Instance sc1, Instance sc2, CostMatrix costMatrix, ParametersContainer pc) {
		// Empty body
	}

	public DTWDistanceInfo onCalculateDTWDistance(Instance sc1, Instance sc2, CostMatrix costMatrix,
	...
	public void afterCalculateDTWDistance(Instance sc1, Instance sc2, CostMatrix costMatrix, ParametersContainer pc) {
		// Empty body
	}
